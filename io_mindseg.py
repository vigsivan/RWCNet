...
from common import MINDSSC, get_identity_affine_grid, apply_displacement_field, \
    MINDSEG  # , apply_displacement_field, get_labels
from metrics import compute_total_registration_error, compute_dice
from io_adam_loops import inst_optimization
...
def apply_instance_optimization(
        ...
        initial_disp_root: Path = Path("./eval_0929"),
        save_directory: Path = Path("./OASIS_labelled_CALR_TEST"),
    ...
    for data in gen:
        ...
        if use_labels:
            maxlabels = max(torch.unique(fixed_segt.long()).shape[0], torch.unique(moving_segt.long()).shape[0])
            weight = 1 / (
                    torch.bincount(fixed_segt.long().reshape(-1), minlength=maxlabels)
                    + torch.bincount(moving_segt.long().reshape(-1), minlength=maxlabels)
            ).float().pow(0.3)
            weight[torch.isinf(weight)] = 0.
            weight /= weight.mean()
            mindssc_fix_ = MINDSEG(fixed_segt, weight)
            mindssc_mov_ = MINDSEG(moving_segt, weight)
        ...
        if data.fixed_segmentation is not None:
            ...
            dice = compute_dice(
                fixed_seg, moving_seg, moved_seg, labels=(fixed_segt.unique()).detach().cpu().numpy() #type: ignore
            )
