...
def apply_instance_optimization(
    ...
    for data in gen:
        ...
        image_name = os.path.basename(data.fixed_image)
        ...
        if data.fixed_segmentation is not None:
            ...
            fixed_segt = torch.tensor(fixed_segmentation).to(device).unsqueeze(0).unsqueeze(0)
            moving_segt = torch.tensor(moving_segmentation).to(device).unsqueeze(0).unsqueeze(0)
        ...
        net = swa_optimization(
            ...
            fs=None, checkpoint_dir=checkpoint_dir,
            fsegt=None,
            msegt=None,
        )
